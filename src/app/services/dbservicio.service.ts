...
export class DbservicioService {
  ...
  tablaViaje: string = "CREATE TABLE IF NOT EXISTS viaje(idviaje INTEGER PRIMARY KEY AUTOINCREMENT, nombre VARCHAR(50) , patente VARCHAR(50) , comuna VARCHAR(50)  , costo INTEGER);";
  ...
  fetchComen(): Observable<Comentario[]> {
  ...
  agregarviaje(nombre,destino,monto,patente){
    let data =[nombre,destino,monto,patente];
    return this.database.executeSql('INSERT or IGNORE INTO viaje(nombre,destino,monto,patente) VALUES (?,?,?,?)',data).then(res=>{
      this.buscarViaje();
